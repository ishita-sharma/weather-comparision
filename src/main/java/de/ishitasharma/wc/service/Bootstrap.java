...
public class Bootstrap implements InitializingBean{
	...
	public void afterPropertiesSet() throws Exception {
		...
		List<City> listOfCities = serializeJsonToObject(formattedCityData);
	...
	private String loadCityData() throws IOException {
		...
		return IOUtils.toString(classLoader.getResourceAsStream(CITY_DATA_FILE));
	}
	...
	private List<City> serializeJsonToObject (String formattedCityData) throws JsonParseException, JsonMappingException, IOException {
		List<City> obj = mapper.readValue(formattedCityData, new TypeReference<List<City>>(){});
		return obj;
	}
	...
	private void storeCityDataInDB (List<City> listOfCities) {
		...
		try(Connection conn = dataSource.getConnection()) {
			...
			for(City city : listOfCities) {
				...
				preparedStatement.setDouble(4, city.getCityCoordinates().getLat());
				preparedStatement.setDouble(5, city.getCityCoordinates().getLon());
	...
	private void stats(int[] results){
		for(int result : results){
			if(result >  0) rowsAdded++;
			if(result == 0)	rowsSkipped++;
		}
	}
	
	private void stats(){
		System.out.println("Data sync summary:" + rowsAdded+" cities added and "+ rowsSkipped + " cities skipped.");
	}
}
